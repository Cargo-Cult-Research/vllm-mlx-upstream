...
#
# StreamingXMLToolCallParser ported verbatim from vLLM PR #25028
# (vllm/entrypoints/openai/tool_parsers/qwen3xml_tool_parser.py,
#  merge commit 9383cd6f).
#
# Qwen3XMLToolParser wrapper adapted for vllm-mlx interface:
#  - dict-based request instead of ChatCompletionRequest
#  - dict-based streaming returns instead of DeltaMessage
#  - ExtractedToolCallInformation with list[dict] instead of list[ToolCall]
#  - Think-tag stripping before parsing (safety against recursive trap)
"""
...
# Type alias — StreamingXMLToolCallParser uses this only in type annotations
...
class _ToolDef:
    ...
    @property
    def type(self) -> str:
        return self._d.get("type", "function")

    @property
    def function(self) -> _FunctionDef:
        return self._func


# ---------------------------------------------------------------------------
# StreamingXMLToolCallParser — from vLLM PR #25028, VERBATIM.
# Only ChatCompletionToolsParam type annotation resolved via alias above.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Qwen3XMLToolParser — adapted wrapper for vllm-mlx interface.
#
# Differences from upstream vLLM:
#  - request is dict | None (not ChatCompletionRequest)
#  - extract_tool_calls returns ExtractedToolCallInformation with list[dict]
#  - extract_tool_calls_streaming returns dict | None (not DeltaMessage)
#  - Strips <think> tags before parsing (safety against recursive trap)
# ---------------------------------------------------------------------------
